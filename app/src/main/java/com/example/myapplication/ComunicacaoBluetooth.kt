...
import com.example.myapplication.ScreenActivity
...
class ComunicacaoBluetooth: Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mHandler=MHandler()
        mConnectThread = ConnectThread(ScreenActivity.mDevice)
    ...
    inner class ConnectThread : Thread {
        ...
        constructor (device: BluetoothDevice) {
            ...
            mmDevice = device;
            try {
                tmp = device.createRfcommSocketToServiceRecord(ScreenActivity.HC05_UUID);
            } catch (e: IOException) {
        ...
        override fun run() {
            mConnectedThread = null;
            ScreenActivity.mBluetoothAdapter!!.cancelDiscovery()
            try {
